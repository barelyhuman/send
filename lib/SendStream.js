...
function SendStream (req, path, options) {
  ...
  if ('staticStore' in opts) {
    this._staticCacheEnabled = true
    this._staticStore = createStore(opts.staticStore.onCleanup)
  }
}
...
SendStream.prototype.stream = function stream (path, options) {
  ...
  // can create a static read stream
  let stream = this.createStaticReadStream(path)
  const fromCache = !!stream
  ...
  stream.pipe(res)
  if (!fromCache && this._staticCacheEnabled) {
    let value
    stream.on('data', (chunk) => {
      value += chunk
    })
    stream.on('end', (chunk) => {
      self._staticStore?.store(path, Buffer.from(value))
    })
  }
...
SendStream.prototype.createStaticReadStream = function (path) {
  if (!this._staticCacheEnabled) {
    return null
  }

  const itemContents = this.getFromStaticCache(path)
  if (!itemContents) {
    return null
  }
  const reader = new Stream.Readable()
  reader.push(itemContents)
  return reader
}

SendStream.prototype.getFromStaticCache = function (path) {
  const cachedData = this._staticStore.retrieve(path)
  if (!cachedData) {
    return false
  }
  return true
